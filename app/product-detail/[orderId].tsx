import { confirmProductOrder, getOrderDetails, updateOrderStatus } from "@/api/product";
...
const ProductDetail = () => {
  ...
  const confirmMutation = useMutation({
    mutationFn: () => confirmProductOrder({ orderId: orderId! }),
    onSuccess: () => {
      showSuccess("Success", "Order confirmed. Payment released to seller.");
      invalidate();
    },
    onError: (error: any) =>
      showError("Error", error?.message ?? "Failed to confirm order"),
  });

  const handleAction = (action: ActionButton) => {
    if (action.usesConfirm) {
            ...
            <View className="flex-1" style={{ position: "relative" }}>
              ...
              >
                ...
                <View className="flex-row items-center justify-between">
                  ...
                  {statusMutation.isPending || confirmMutation.isPending ? (
                  ...
                  >
                    {actions.map((action, index) => (
                      <React.Fragment key={action.newStatus}>
                        ...
                        <TouchableOpacity
                          activeOpacity={0.6}
                          disabled={statusMutation.isPending || confirmMutation.isPending}
                          onPress={() => {
                            setMenuOpen(false);
                            handleAction(action);
                          }}
                        ...
                        >
                          ...
                          {(statusMutation.isPending &&
                            statusMutation.variables?.newStatus ===
                              action.newStatus) ||
                          (action.usesConfirm && confirmMutation.isPending) ? (
                            <ActivityIndicator
                              size="small"
                              color={action.color}
                            />
