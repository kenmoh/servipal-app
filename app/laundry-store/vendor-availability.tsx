...
//             <View className="bg-red-500/10 border border-red-500/30 rounded-full items-center p-3 gap-2">
...
//                   <TouchableOpacity
//                     onPress={() => setExpandedDay(isExpanded ? null : dayId)}
//                     activeOpacity={0.7}
...
const VendorAvailability = () => {
  ...
  // Populate form from existing availability data
  useEffect(() => {
    ...
    data.forEach((slot: any) => {
      const dayId: number = slot.day_of_week;
      ...
      dayConfigs[dayId] = {
        ...
        isExpress: slot.is_express, // ✅ per-day
        expressFee: slot.express_fee, // ✅ per-day
      };
    });

    setFormData({ selectedDays, dayConfigs });
  }, [data]);
  ...
  const toggleDay = (dayId: number) => {
    ...
    if (isSelected) {
      ...
      updatedDays = [...selectedDays, dayId];
      updatedConfigs[dayId] = {
        ...DEFAULT_DAY_CONFIG,
        isExpress: false, // default off for new days
        expressFee: 0,
      };
      setExpandedDay(dayId);
    }
  ...
  const onSubmit = async () => {
    ...
    // NOTE: update validateAvailabilityForm in your utils to drop the
    // global express params — validation is now purely per-day via dayConfigs
    const validation = validateAvailabilityForm(
      selectedDays,
      formData.dayConfigs,
    ...
    setIsLoading(true);
    try {
      // NOTE: update generateAvailabilityInputs to read isExpress/expressFee
      // from each dayConfig entry instead of a global flag
      const slots = generateAvailabilityInputs(
        vendorId || user?.id || "",
        "VENDOR_DELIVERY",
        selectedDays.sort((a, b) => a - b),
            ...
            .map((dayId) => {
              ...
              return (
                <View
                  key={dayId}
                  className="rounded-xl overflow-hidden border border-gray-200"
                  style={[nativeTheme === "dark" && { borderColor: "#333" }]}
                >
                  {/* Day Header */}
                  <TouchableOpacity
                    onPress={() => setExpandedDay(isExpanded ? null : dayId)}
                  ...
                  {isExpanded && (
                    <View
                      className="p-4 gap-4 border-t border-gray-200"
                      style={[
                        nativeTheme === "dark" && { borderColor: "#333" },
                      ]}
                    >
                      {/* Operating Hours */}
                      <View className="gap-2">
                      ...
                      {/* ✅ Per-Day Express Toggle */}
                      <View
                        className="gap-3 bg-gray-50 rounded-lg p-3"
                        style={[
                          nativeTheme === "dark" && {
                      ...
                      >
                        <View className="flex-row items-center justify-between">
                          ...
                          <TouchableOpacity
                            ...
                            style={{
                              backgroundColor: config.isExpress
                                ? "orange"
                                : nativeTheme === "dark"
                                  ? "#333"
                                  : "#D1D5DB",
                        ...
                        {config.isExpress && (
                          <AppTextInput
                            ...
                            onChangeText={(text) =>
                              setDayConfig(dayId, {
                                expressFee: Number(text) || 0,
                              })
                            }
                            value={config.expressFee?.toString()}
                            width="100%"
                          />
                        )}
                      </View>
          ...
          {selectedDays.length > 0 && (
            ...
            >
              <View className="flex-row items-center gap-2">
                ...
                >
                  You will update {selectedDays.length} availability slot
                  {selectedDays.length > 1 ? "s" : ""}
                </Text>
              </View>
              <Text
