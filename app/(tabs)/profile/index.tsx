...
const ProfileScreen = () => {
  const {
    ...
    biometricEnabled,
    setBiometricEnabled: setStoreBiometric,
  } = useUserStore();
  ...
  const handleBiometricToggle = async (newValue: boolean) => {
    try {
      ...
      if (newValue) {
        // Prompt to confirm before enabling
        const success = await promptBiometric("Confirm fingerprint setup");
        if (!success) {
          return;
        }
      }
      ...
      <View className="mt-6 px-3">
        <View className="bg-surface-elevated rounded-2xl p-4 gap-4">
          ...
          <ToggleSwitch
            ...
            icon={<Ionicons name="finger-print-outline" size={20} color="gray" />}
