import { createPayoutAccount, fetchBanks, fetchPayoutAccounts, resolveBank } from "@/api/user";
...
const CreatePayoutAccountScreen = () => {
  ...
  const { data: existingPayoutAccount } = useQuery({
  ...
  // Populate form with existing payout account data
  useEffect(() => {
    if (existingPayoutAccount) {
      setValue("bank_name", existingPayoutAccount.bank_name, {
        shouldValidate: true,
      });
      setValue("account_number", existingPayoutAccount.account_number, {
        shouldValidate: true,
      });
      setValue("account_bank", existingPayoutAccount.bank_code, {
        shouldValidate: true,
      });
      setValue("beneficiary_name", existingPayoutAccount.full_name || "", {
        shouldValidate: true,
      });
    }
  }, [existingPayoutAccount, setValue]);
        ...
        >
          <View className="px-5 mt-5 gap-6">
            ...
            {existingPayoutAccount && (
              <View className="bg-background rounded-lg p-4 gap-3 border border-orange-300/50">
                <View className="gap-1">
                  <Text className="text-slate-500 font-poppins-regular text-sm">
                    Beneficiary ID
                  </Text>
                  <Text className="text-secondary font-poppins-semibold text-base">
                    {existingPayoutAccount.beneficiary_id}
                  </Text>
                </View>
              </View>
            )}
            ...
            <View className="gap-5">
              ...
              <View className="mt-5 mb-10">
                <AppButton
                  text={existingPayoutAccount ? "Update Account" : "Create Account"}
                  variant="fill"
                  icon={
