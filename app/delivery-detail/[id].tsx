...
const ItemDetails = () => {
  ...
  const normalizeCoords = (coords: any): [number, number] | null => {
    if (!coords || !Array.isArray(coords) || coords.length < 2) {
      console.warn("⚠️ normalizeCoords: invalid input", {
        coords,
        type: typeof coords,
  ...
  useEffect(() => {
    if (data) {
      if (data.pickup_location && data.pickup_coordinates) {
        ...
        console.warn("⚠️ Missing pickup data:", {
      ...
      if (data.destination && data.dropoff_coordinates) {
        ...
        console.warn("⚠️ Missing dropoff data:", {
  ...
  const getSmartActionButton = () => {
    ...
    const handleAction = (btnConfig: any) => {
      ...
      if (
        btnConfig.nextStatus === "ACCEPTED" ||
        btnConfig.nextStatus === "PICKED_UP"
      ) {
