...
const mapUserTransaction = (tx: any): TransactionDetails => ({
  ...tx,
...
export const fetchCurrentUserTransactions = async (
  ...
  // get_user_transactions uses auth.uid() internally (SECURITY DEFINER), so the
  // current user's rows are guaranteed; it also dedupes each order to one entry.
  ...
  return {
    data: (result.transactions ?? []).map(mapUserTransaction),
    total: result.total_count ?? 0,
    total_spent: result.total_spent ?? 0,
    total_payout: result.total_payout ?? 0,
    pending_payout: result.pending_payout ?? 0,
  };
};

export const fetchTransactionSummary = async (): Promise<TransactionSummaryResponse> => {
  try {
    // Ensure user is authenticated
    const { data: session, error: sessionError } =
      await supabase.auth.getSession();
    ...
    // Get transactions + payout summary via rpc
    const { data, error } = await supabase.rpc("get_user_transactions", {
      p_limit: 50,
      p_offset: 0,
    });
    ...
    const result = (data ?? {}) as TransactionSummaryResponse;

    return {
      ...result,
      transactions: (result.transactions ?? []).map(mapUserTransaction),
    };
  } catch (error) {
    throw error;
  }
...
export const createPayoutAccount = async (payoutAccount: PayoutAccount): Promise<PayoutResponse> => {
  try {
...
type PayoutAccountResponse = Pick<
  Beneficiary,
  "beneficiary_id" | "bank_name" | "bank_code" | "account_number" | "full_name"
>;

export const fetchPayoutAccounts = async (): Promise<PayoutAccountResponse | null> => {
  const { data: session } = await supabase.auth.getSession();
  if (!session.session) {
    throw new Error("User not authenticated");
  }

  const beneficiaryId = session.session.user.id;

  const { data, error } = await supabase
    .from("beneficiaries")
    .select("beneficiary_id, bank_name, bank_code, account_number, full_name")
    .eq("id", beneficiaryId)
    .maybeSingle();
